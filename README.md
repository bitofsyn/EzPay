...
EzPay는 사용자의 금융 계좌를 연결해 실제 거래 데이터를 수집하고, 이를 정규화·분류·분석하여 소비 패턴 변화와 이상 지출 징후를 자동으로 설명해주는 개인 금융 분석 서비스입니다. 단순한 가계부 UI가 아니라, 외부 금융 API 연동, 거래 동기화 파이프라인, 이벤트 기반 분석, AI 설명 레이어까지 포함한 금융 데이터 제품을 목표로 합니다.
...
- 실제 금융 API 또는 금융 API와 동일한 연동 구조를 갖는 거래 수집 파이프라인 구축
...
핵심 메시지:
`EzPay는 사용자의 실제 거래 데이터를 연결하고 해석하는 개인 금융 분석 앱이다.`
...
- 실제 거래 데이터 연동을 위한 connector layer 추가
- Plaid Sandbox 기반 거래 동기화 PoC 구현
- KFTC Open Banking 조회형 연동 구조 검토
- 거래 정규화 모델 도입
