...
public class NotificationController {
    ...
    // 사용자 알림 설정 조회
    @GetMapping("/{userId}")
    public ResponseEntity<CommonResponse<List<NotificationInfo>>> getUserNotifications(@PathVariable Long userId) {
        try {
            List<NotificationInfo> notifications = notificationService.getUserNotifications(userId);
    ...
    // 사용자 알림 설정 변경
    @PutMapping("/{userId}")
    public ResponseEntity<CommonResponse<NotificationInfo>> updateUserNotifications(@PathVariable Long userId,
                                                                                        @Valid @RequestBody NotificationSettingRequest notificationRequest) {
        try {
            NotificationInfo response = notificationService.updateNotification(userId, notificationRequest);
