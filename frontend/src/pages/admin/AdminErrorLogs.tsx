...
const AdminErrorLogs: React.FC = () => {
  ...
  const fetchErrorLogs = useCallback(async (): Promise<void> => {
    setIsLoading(true);
    try {
      let data: ErrorLog[];
      if (statusFilter === "ALL") {
        data = await getAllErrorLogs();
      } else {
        data = await getErrorLogsByStatus(statusFilter);
      }
      setErrorLogs(data);
    } catch (error) {
  ...
  const handleResolve = async (logId: number): Promise<void> => {
  ...
  const getStatusBadge = (status: ErrorLog["status"]): JSX.Element => {
    if (status === "RESOLVED") {
      return (
