...
const AdminUserDetail: React.FC = () => {
  ...
  const handleStatusChange = async (newStatus: UserStatus): Promise<void> => {
    if (!userId) return;
    ...
    try {
      await updateUserStatus(numericUserId, newStatus);
  ...
  const getStatusBadgeStyle = (status: UserStatus): string => {
    switch (status) {
      case "ACTIVE":
